...
#View(MaleMP)
MaleMP <- read_excel(FileName, sheet = 'MP-2019_Male')
FemaleMP <- read_excel(FileName, sheet = 'MP-2019_Female')
...
scale.act.male <- 0.92 #(0.92 for NDPERS)
scale.ret.male <- 1.03 #(1.03 for NDPERS)
scale.act.female <- 0.92 #(0.92 for NDPERS)
scale.ret.female <- 1.01 #(1.01 for NDPERS)
...
colnames(SurvivalRates)[4] <- "PubG_2010_employee_male"
colnames(SurvivalRates)[2] <- "PubG_2010_healthy_retiree_male"
colnames(SurvivalRates)[7] <- "PubG_2010_employee_female"
colnames(SurvivalRates)[5] <- "PubG_2010_healthy_retiree_female"
...
MortalityTable <- MortalityTable %>% 
  ...
  mutate(MPcumprod_male = cumprod(1 - MaleMP_final)/(1 - MaleMP_final[Years == 2010]),
         ...
         mort_male = ifelse(IsRetirementEligible(Age, YOS) == F, PubG_2010_employee_male * MPcumprod_male * ScaleMultipleMaleAct,
                            PubG_2010_healthy_retiree_male * MPcumprod_male * ScaleMultipleMaleRet),
         mort_female = ifelse(IsRetirementEligible(Age, YOS) == F, PubG_2010_employee_female * MPcumprod_female * ScaleMultipleFemaleAct ,
                              PubG_2010_healthy_retiree_female * MPcumprod_female) * ScaleMultipleFemaleRet,
...
SeparationRates$SepRateMale <- as.numeric(SeparationRates$SepRateMale)
SeparationRates$SepRateFemale <- as.numeric(SeparationRates$SepRateFemale)
...
SeparationRates <- SeparationRates %>% 
  mutate(retirement_cond = IsRetirementEligible(Age,YOS),
         SepRateMale = ifelse(retirement_cond == T, "Retirement Rate",
                              ifelse(YOS < 5, "Termination before 5 Male" , "Termination after 5 Male")),
         SepRateFemale = ifelse(retirement_cond == T,"Retirement Rate",
                                ifelse(YOS < 5, "Termination before 5 Female", "Termination after 5 Female")),
         SepRate = (SepRateAvg)) %>% 
  group_by(entry_age) %>% 
...
ReducedFactor <- ReducedFactor %>% 
  mutate(RF = ifelse(RetirementAge >= NormalRetAgeI & YOS >= NormalYOSI |
                     ...
                     ifelse((YOS + RetirementAge >= ReduceRetRule)  & YOS >= ReduceRetAge & YOS >= ReduceRetYOS, pmin(1 - (2/3*12/100)*(NormalRetAgeI - RetirementAge),1), 0)))
#View(ReducedFactor)
##reduced by 2/3% of one percent for each month before the earlier of age 65 or the age at which the Rule of 90 is met.
